...
<head>
    ...
    <title>Voter LogIn | Cast Votes</title>
  ...
  <div class="container">
      <div class="row justify-content-center">
        <div class="col-lg-5">
          ...
          <h5><a href="Homepage.php"><img src="images/logo.png" class="img-responsive" /></a>VOTER LOGIN (CAST VOTES)</h5>
    ...
    if (empty($sid)) {
        ...
        if (mysqli_num_rows($result) === 1) {
            ...
            if ($row['Student_ID'] === $sid && $row['Serial_Code'] === $scode) {
                ...
                if (mysqli_num_rows($result) === 1) {
                    $row = mysqli_fetch_assoc($result);
                    if ($row['Voting_Status'] === $Vstatus) {
                        $_SESSION['status'] = "Login Successful!";
                        $_SESSION['status_code'] = "success";
                        header("Location: StudentLogin_Castvotes.php");
                        ob_end_flush();
                        exit();
                    } else{
                        $_SESSION['status'] = "Login Successful!";
                        $_SESSION['status_code'] = "success";
                        header("Location: Studentlogin_VoteClosed.php");
                        ob_end_flush();
                        exit();
                    }
                }
                  ...
                  $_SESSION['status'] = "Access Denied. You have Already Voted";
