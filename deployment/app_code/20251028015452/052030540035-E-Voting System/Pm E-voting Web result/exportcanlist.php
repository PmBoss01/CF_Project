...
// Export Candidate List
    $query = $conn->query("SELECT * FROM candidate_details");
    ...
    if ($query->num_rows > 0) {
        ...
        $activity_log = "Candidates List Exported";
        ...
        $delimiter = ",";
        $filename = "Candidates-List_" . date('Y-m-d') . ".csv";

        // Create a file pointer
        $f = fopen('php://memory', 'w');

        //set column headers
        $field = array('CANDIDATE ID', 'CANDIDATE NAME', 'PORTFOLIO', 'CONTACT', 'DEPARTMENT');
        fputcsv($f, $field, $delimiter);

        //Output for each row
        while ($row = $query->fetch_assoc()) {
            $lineData = array($row['Student_ID'], $row['Student_Name'], $row['Position'], $row['Contact'], $row['Department']);
