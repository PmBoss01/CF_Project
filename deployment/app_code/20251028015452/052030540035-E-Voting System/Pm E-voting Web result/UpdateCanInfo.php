...
<html lang="en">
  <head>
    ...
    <title>Candidate | Edit Info</title>
...
<a href="Notificationbell_conn.php?id=123" target="_blank" name="bell">
    ...
    <span class="visually-hidden">unread messages</span>
      ...
      <ul class="mt-2">
          ...
          <li>
            ...
            <li><a class="dropdown-item" href="managecandidate.php">Candidate</a></li>
  ...
  <div class="container-fluid bg-light" style="margin-top: 50px;margin-bottom: 30px;">
    <div class="card shadow mt-3">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-black"><i class="fa fa-user-check mx-1"></i>EDIT CANDIDATE INFORMATION</h6>
        </div>
        <div class="card-body">
            <?php
            if(isset($_POST['caneditbtn'])){
                $cid = $_POST['editcid'];

                $query = "SELECT * FROM candidate_details WHERE Student_ID  = '$cid'";
                $query_run = mysqli_query($conn, $query);

                foreach ($query_run as $row) {
                  ?>
                <form action="edit_deleteCan.php" method="post" enctype="multipart/form-data">
                <input type="hidden" name="editcid" value="<?php echo $row['Student_ID'] ?>">

                <label for="id" class="form-label fw-bold fs-6 m-2" >Candidate ID</label>
                <p class="text-success m-2 fs-4 mt-0 mb-0 fw-bold"><?php echo $row['Student_ID'] ?></p>

            <label for="name" class="form-label fw-bold fs-6 m-2" >Candidate Name</label>
           <input type="text" class="form-control mb-0" name="sname" id="validationDefault01" value="<?php echo $row['Student_Name']?>" required>
            
           <label for="validationDefault01" class="form-label fw-bold fs-6 m-2" >Contact</label>
           <input type="text" class="form-control mb-0" id="validationDefault01" name="pno" oninput="this.value = this.value.replace(/[^0-9]/g, '')" maxLength="10" value="<?php echo $row['Contact']?>" required>

           <label for="position" class="form-label fw-bold fs-6 m-2" >Reselect Portfolio</label>
           <select id="validationDefault04" class="form-select" name="position" required>
            <option selected disabled value=""><?php echo $row['Position']?></option>
                <?php include 'Portfolio_SelectConn.php'; ?>
           </select>

           <label for="validationDefault04" class="form-label fw-bold fs-6 m-2" >Reselect Department</label>
           <select id="validationDefault04" class="form-select" name="Department" required>
           <option selected disabled value="">~select~</option>
                <?php include 'Department_SelectConn.php'; ?>
           </select>

           <label for="validationDefault01" class="form-label fw-bold fs-6 m-2" >Reupload Candidate Photo</label>
           <input type="file" class="form-control mb-1" name="image" id="imageInput" onchange="displayImage(event)"id="photos" accept=".png,.jpg,.jpeg" required>
          ...
          <img src="" alt="Upload Image" id="previewImage" style="width: 135px;height:135px">
            <button type="submit" name="updatecaninfo" class="btn btn-primary">Update</button>
          </div>
                </form>
            <?php
                }
